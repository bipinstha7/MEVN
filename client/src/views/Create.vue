<template>
	<div class="login-form">
		...
		<div class="form-group">
			<!-- <input type="file" id="file" ref="file" v-on:change="handleImageUpload()"> -->
			<input type="file" name="image" ref="file" accept="image/*" @change="handleImageUpload">
		</div>
		<div>
			<img :src="`http://localhost:3000/${imageUrl}`" height="150" v-if="updateImage && this.imageUrl">
			<img :src="imageUrl" height="150" v-else>
		</div>
		<div class="form-group">
			<button @click="edit" class="btn btn-primary btn-block" v-if="$route.params.id">Edit</button>
			<button @click="create" class="btn btn-primary btn-block" v-else>Create</button>
		</div>
		<div class="clearfix">
